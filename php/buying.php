...
<!-- Content -->
  ...
  <section class="function" style="text-align: center;">
	<div id="info" class="textcenter">
  
	
    <h2 id="italic"> Buying result </h2>
    <?php
    $uid = $_GET['uid'];
    $title = $_POST['title'];
    echo '<h4>Here is our result!</h4>';
    
    $mysqli = new mysqli('localhost','root','','bookbig','3307');
    if($mysqli->connect_errno){
        echo $mysqli->connect_errno;
    }
    else{$q = "SELECT * FROM book_sell_detial WHERE title = '$title' ORDER BY price";
        if($result = $mysqli->query($q)){
            $result->data_seek(0);
            $row = $result->fetch_array();
            if(!isset($row)){
                echo "There is no '$title' here";
            }
            else{
                $q = "SELECT * FROM userinfo WHERE UserID = $row[from_user]";
                if($result = $mysqli->query($q)){
                    $result->data_seek(0);
                    $row_user = $result->fetch_array();
                echo "'$title' is available from $row_user[fname] $row_user[lname]";
                echo "<br>with price $row[price] (the cheapest)<br>";
                echo "<br>Please contact<br>";
                echo "'$row_user[tel]'<br> or <br> '$row_user[email]'";
                $amount =  $row['amount']-1;
                if($amount != 0){
                echo "Anyway we have $amount more for you!";}
                }
